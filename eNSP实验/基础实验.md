...
<!-- 截图：实验拓扑全景图（LSW + AR + PC8~PC11） -->

图中左侧是接入层交换机（LSW），右侧是核心层路由器（AR）。LSW 负责划分 VLAN，AR 负责配置 Vlanif 实现跨 VLAN 路由。只有两者配合，PC8 才能成功 ping 通 PC9 / PC10 / PC11。
...
<!-- 截图：AR 上 display ip interface brief 的输出 -->
...
<!-- 截图：Wireshark 或 eNSP 抓包，展示 802.1Q 标签 -->
