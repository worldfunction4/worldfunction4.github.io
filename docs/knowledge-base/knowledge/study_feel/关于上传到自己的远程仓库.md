...
```mermaid
flowchart TB
  subgraph root["Socket 根目录"]
    client["socket_client"]
    server["socket_server"]
    gi[".gitignore"]
  end
  subgraph nested["socket_domo1"]
    lock["蓝锁 = 已被另一个 Git 仓库跟踪"]
  end
  server --> nested
```
...
这里我运行后得到了以下输出：误把 `.gitignore` 建成了目录。

```mermaid
flowchart TD
  A["git status"] --> B{"出现 .gitignore/ 目录?"}
  B -->|是| C["删掉这个文件夹"]
  C --> D["改成真正的 .gitignore 文件"]
  B -->|否| E["继续绑定远程"]
```
...
```mermaid
flowchart LR
  L["本地 main"] -->|"git push -u origin main"| R["GitHub origin"]
```
...
```mermaid
flowchart LR
  origin_fetch["origin fetch"] --> URL["GitHub 仓库 URL"]
  origin_push["origin push"] --> URL
```
