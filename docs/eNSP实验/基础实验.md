---
title: 华为网络实战：从子网逻辑到跨 VLAN 路由（eNSP 篇）
date: 2026-04-22
category:
  - 计算机网络
tag:
...
flowchart TB
  ...
  LSW --> V1["VLAN 1 默认"]
  LSW --> V10["VLAN 10"]
  LSW --> V20["VLAN 20 已创建未绑定端口"]
  V1 --- P2["Eth0/0/2 Up/UT"]
  V10 --- P1["Eth0/0/1 Up/UT"]
  V10 --- P3["Eth0/0/3 Up"]
```
...
```
`display ip interface brief` 验证 Vlanif 已起来：

```mermaid
flowchart LR
  AR3[AR3]
  AR3 --> E08["Ethernet0/0/8 未分配 down"]
  AR3 --> N0["NULL0 up spoofing"]
  AR3 --> V10["Vlanif10 192.168.1.1/24 up"]
```
...
### 华为设备中非常坑人的硬件特性差异
- **AR201**：右侧口（LAN 口）默认是二层交换模式，无法直接配 IP 地址，必须使用 Vlanif 方案。
- **AR2220**：所有口都支持三层路由模式，可以直接配 IP 地址。
